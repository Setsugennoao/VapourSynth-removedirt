...
static void postprocessing(
    PostProcessingData *pp, uint8_t *dp, int32_t dpitch, uint8_t *dpU, uint8_t *dpV, int32_t dpitchUV,
    ...
    do {
      ...
      do {
        if ((cl[0] & TO_CLEAN) != 0) {
          ...
          if (cl[-1] == 0) {
            if ((vertical_diff(dp2 + leftdp, dpitch, pp->mdd.md.noiselevel) > vertical_diff(sp2 + leftsp, spitch, pp->mdd.md.noiselevel) + pp->pthreshold) || (pp->vertical_diff_chroma(dpU2 + Cleftdp, dpV2 + Cleftdp, dpitchUV, pp->mdd.md.noiselevel) > pp->vertical_diff_chroma(spU2 + Cleftsp, spV2 + Cleftsp, spitchUV, pp->mdd.md.noiselevel) + pp->cthreshold)) {
              ++to_restore;
              cl[-1] = MOTION_FLAG3;
            }
          }
          ...
          if (cl[1] == 0) {
            if ((vertical_diff(dp2 + rightdp, dpitch, pp->mdd.md.noiselevel) > vertical_diff(sp2 + rightsp, spitch, pp->mdd.md.noiselevel) + pp->pthreshold) || (pp->vertical_diff_chroma(dpU2 + Crightdp, dpV2 + Crightdp, dpitchUV, pp->mdd.md.noiselevel) > pp->vertical_diff_chroma(spU2 + Crightsp, spV2 + Crightsp, spitchUV, pp->mdd.md.noiselevel) + pp->cthreshold)) {
              ++to_restore;
              cl[1] = MOTION_FLAG3;
            }
          }
          ...
          if (cl[pp->mdd.md.pline] == 0) {
            if ((horizontal_diff(dp2 + topdp, dpitch) > horizontal_diff(sp2 + topsp, spitch) + pp->pthreshold) || (horizontal_diff_chroma(dpU2 + Ctopdp, dpV2 + Ctopdp, dpitchUV) > horizontal_diff_chroma(spU2 + Ctopsp, spV2 + Ctopsp, spitchUV) + pp->cthreshold)) {
              ++to_restore;
              cl[pp->mdd.md.pline] = MOTION_FLAG3;
            }
          }
          ...
          if (cl[pp->mdd.md.nline] == 0) {
            if ((horizontal_diff(dp2 + bottomdp, dpitch) > horizontal_diff(sp2 + bottomsp, spitch) + pp->pthreshold) || (horizontal_diff_chroma(dpU2 + Cbottomdp, dpV2 + Cbottomdp, dpitchUV) > horizontal_diff_chroma(spU2 + Cbottomsp, spV2 + Cbottomsp, spitchUV) + pp->cthreshold)) {
              ++to_restore;
              cl[pp->mdd.md.nline] = MOTION_FLAG3;
            }
          }
